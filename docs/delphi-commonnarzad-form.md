...
| Вкладка «Задание»: дерево 75% / параметры + алгоритм 25% | **сделано** |
| Данные дерева (qrNarZad), параметры (qrParamZ), алгоритм (qrAlgInfo) | **сделано** (read-only, дерево свёрнуто) |
| Расчёт, закрытие, правка структуры | отложено |
