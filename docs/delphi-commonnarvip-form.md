...
Список нарядов и вход на карточку: [naryady-notes.md](./naryady-notes.md).

## Веб-статус (burnar)

| Часть | Статус |
|-------|--------|
| Открытие с списка (`/naryad/:id`) | **сделано** |
| Action bar: вкладки + «Наряд - id имя» + заглушка параметров; каталог — модалка | **сделано** |
| Вкладка «Выполнение»: дерево 75% / параметры + алгоритм 25% | **сделано** (колонки плюс «Факт» и «Период») |
| Данные дерева (qrNarVip), параметры (qrParamV), алгоритм (qrAlgInfo) | **сделано** (read-only, дерево свёрнуто) |
| Правка факта, периоды, блокировки | отложено |

В Delphi `MainUnit.OpenNar` открывает задание и выполнение как два MDI-окна. В вебе это одна страница `/naryad/:id` с двумя вкладками.
