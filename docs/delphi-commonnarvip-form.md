...
| Вкладка «Выполнение»: дерево 75% / параметры + алгоритм 25% | **сделано** (таблицы пустые, без API; колонки плюс «Факт» и «Период») |
| Данные дерева, факт, периоды, блокировки | отложено |
