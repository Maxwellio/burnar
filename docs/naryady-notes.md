...
| Action bar (подпись наряда, параметры, каталог-модалка, выход на список) | **сделано** |
...
| Справа | заглушка общих параметров; **«Каталог»** открывает read-only дерево тематических разделов (раскрытие и фильтры помнятся, пока открыта карточка); **«Выйти из наряда»** (`Close`) → `navigate('/')` |
...
| `frontend/src/pages/NaryadPage.jsx` | карточка: action bar + две панели + CatalogDialog |
| `frontend/src/pages/CatalogDialog.jsx` | модальный каталог тематических разделов |
